...
//     int bw1;
	...
	QSqlQuery query("SELECT `uid` , `framed_ip` FROM `users` WHERE `gid` = '14' ");
	...
	while (query.next()) 
	{
		    ...
		    QString bw2 = query.value(2).toString();	 	 
	            ...
	            exec_this ="/sbin/ipfw delete 1";
		    exec_this +=num;
		    exec_this +=" flush ";
    		    system(exec_this.toStdString().c_str());		    		
		
	            exec_this ="/sbin/ipfw pipe 1";
		    exec_this +=num;
		    exec_this +=" config mask dst-ip 0x000000ff bw 8000";
		    exec_this +="Kbit/s";
    		    system(exec_this.toStdString().c_str());		    		
